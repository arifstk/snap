...
//   return (
//     <div className='flex flex-col h-125 rounded-2xl overflow-hidden shadow-xl border border-gray-200 mt-4'>

//       {/* ✅ Header — shows delivery boy avatar + name */}
//       <div className='flex items-center gap-3 px-4 py-3 bg-[#075E54]'>
//         <Avatar name={deliveryBoy.name} image={deliveryBoy.image} />
//         <div>
//           <p className='text-white font-semibold text-sm'>{deliveryBoy.name}</p>
...
//           const senderInfo = isMe ? customer : deliveryBoy;
...
const UserChat = ({ orderId, userId }: Props) => {
  const [newMessage, setNewMessage] = useState('');
  const [messages, setMessages] = useState<Message[]>([]);
  // CHANGELOG: customer = the person the user (customer) is chatting with header perspective
  const [customer, setCustomer] = useState<PersonInfo>({ name: 'Me' });
  const [deliveryBoy, setDeliveryBoy] = useState<PersonInfo>({ name: 'Delivery Boy' });
  const bottomRef = useRef<HTMLDivElement>(null);
  ...
  const sendMsg = () => {
    ...
    const message: Message = {
      roomId: orderId,
      text: newMessage,
      // CHANGELOG: store userId as plain string to match normalized senderIds
  ...
  return (
    <div className='flex flex-col h-125 rounded-2xl overflow-hidden shadow-xl border border-gray-200 mt-4'>
      ...
      {/* Header — shows delivery boy avatar + name (who customer is chatting with) */}
      <div className='flex items-center gap-3 px-4 py-3 bg-[#075E54]'>
        <Avatar name={deliveryBoy.name} image={deliveryBoy.image} />
        <div>
          <p className='text-white font-semibold text-sm'>{deliveryBoy.name}</p>
      ...
      >
        ...
        {messages.map((msg, i) => {
          ...
          // Previously crashed with: "can't access property toString, msg.senderId is undefined"
          const isMe = msg.senderId != null && msg.senderId.toString() === userId.toString();
          // isMe=true → customer sent it → show on RIGHT with customer avatar
          // isMe=false → delivery boy sent it → show on LEFT with deliveryBoy avatar
          // CHANGELOG: fixed senderInfo — was `isMe ? customer : deliveryBoy` which was wrong
          // for customer view: when isMe, show customer avatar on right; else deliveryBoy on left
          const senderInfo = isMe ? customer : deliveryBoy;
