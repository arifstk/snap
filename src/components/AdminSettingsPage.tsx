...
//   const [bannerDraft, setBannerDraft] = useState({ id: 0, title: '', subtitle: '', btnText: '', bg: '', link: '' });
...
//       websiteName: 'Snap', deliveryFee: 40, freeDeliveryThreshold: 100,
...
//   };
//   const addNewBanner = () => {
//     const newSlide = { id: Date.now(), title: 'New Slide', subtitle: 'Subtitle here', btnText: 'Shop Now', bg: '', link: '/' };
...
//                           <Field label="Background Image" hint="Upload a file or paste a URL.">
//                             <BannerImageUploader
//                               currentUrl={bannerDraft.bg}
//                               onUpload={(url) => setBannerDraft({ ...bannerDraft, bg: url })}
//                             />
//                           </Field>
...
//             {activeSection === 'banner' && (
//               <SectionCard key="banner" title="Hero Banner Slides" icon={<Image className="w-5 h-5" />}>
//                 <p className="text-xs text-gray-400 -mt-2">Edit slides shown on the homepage hero carousel.</p>
//                 <div className="space-y-3">
//                   {/* YOU MUST MAP HERE SO 'slide' IS DEFINED */}
//                   {draft.bannerSlides.map((slide) => (
//                     <div key={slide.id} className="border border-gray-100 rounded-xl overflow-hidden">
//                       {editingBannerId === slide.id ? (
//                         <div className="p-4 space-y-3 bg-green-50/40">
...
//                           <Field label="Button Link (URL)" hint="e.g. /shop">
//                             <Input value={bannerDraft.link} onChange={(v) => setBannerDraft({ ...bannerDraft, link: v })} placeholder="/category/organic" />
//                           </Field>
...
//                           </Field>
//                           <div className="flex gap-2 pt-1">
//                             <button onClick={saveBannerEdit} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-xs rounded-lg font-medium">
//                               <Check className="w-3.5 h-3.5" /> Save Slide
//                             </button>
//                             <button onClick={() => setEditingBannerId(null)} className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 text-gray-600 text-xs rounded-lg">
//                               <X className="w-3.5 h-3.5" /> Cancel
//                             </button>
//                           </div>
//                         </div>
...
//                             <div className="min-w-0">
//                               <p className="text-sm font-medium text-gray-800 truncate">{slide.title}</p>
//                               <p className="text-[10px] text-blue-500 truncate">{slide.link}</p>
//                             </div>
//                           </div>
//                           <div className="flex items-center gap-1 shrink-0 ml-2">
//                             <button onClick={() => startEditBanner(slide)} className="p-1.5 text-gray-400 hover:text-green-600 rounded-lg"><Edit3 className="w-4 h-4" /></button>
//                             <button onClick={() => removeBanner(slide.id)} className="p-1.5 text-gray-400 hover:text-red-500 rounded-lg"><Trash2 className="w-4 h-4" /></button>
//                           </div>
//                         </div>
//                       )}
//                     </div>
//                   ))}
//                 </div>
//                 <button onClick={addNewBanner} className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-green-200 text-green-600 text-sm font-medium rounded-xl hover:bg-green-50 mt-4">
//                   <Plus className="w-4 h-4" /> Add New Slide
//                 </button>
//               </SectionCard>
//             )}
...
const AdminSettingsPage = () => {
  ...
  // Local "Draft" state handles inputs before saving to the database
  const [draft, setDraft] = useState<ISettings>(settings);
  const [editingBannerId, setEditingBannerId] = useState<number | null>(null);
  const [bannerDraft, setBannerDraft] = useState({ id: 0, title: '', subtitle: '', btnText: '', bg: '', link: '' });
  ...
  // Keep draft in sync if external settings change
  ...
  const handleSave = async () => {
    try {
      // Optimistically update local redux state, then sync with DB
      dispatch(setLocalSettings(draft));
      const result = await dispatch(saveSettings(draft));
      if (saveSettings.fulfilled.match(result)) {
        toast.success('Settings saved successfully!');
  ...
  const handleReset = async () => {
    ...
    await dispatch(saveSettings({ ...draft, ...defaults }));
    toast.success('Settings reset to defaults');
    setResetConfirm(false);
  };
  ...
  const saveBannerEdit = () => {
    updateDraft({
      bannerSlides: draft.bannerSlides.map((s) => (s.id === bannerDraft.id ? bannerDraft : s))
    });
    setEditingBannerId(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <Link href="/" className="inline-flex items-center gap-1 text-gray-500 hover:text-green-600 text-xs font-medium transition-all">
          <ChevronRight className="w-3.5 h-3.5 rotate-180" /> Back to Storefront
        </Link>
      </div>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-12 flex flex-col md:flex-row gap-6">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeSection === 'banner' && (
              <SectionCard key="banner" title="Hero Banner Slides" icon={<Image className="w-5 h-5" />}>
                      ...
                      {editingBannerId === slide.id ? (
                        <div className="p-4 space-y-3 bg-green-50/40">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <Field label="Button Text"><Input value={bannerDraft.btnText} onChange={(v) => setBannerDraft({ ...bannerDraft, btnText: v })} /></Field>
                            <Field label="Link URL"><Input value={bannerDraft.link} onChange={(v) => setBannerDraft({ ...bannerDraft, link: v })} placeholder="/shop" /></Field>
                          </div>
                          <Field label="Background Image">
                            <BannerImageUploader
                              currentUrl={bannerDraft.bg}
                        ...
                        <div className="flex items-center justify-between p-4">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-gray-800 truncate">{slide.title}</p>
                              <p className="text-[10px] text-blue-500 truncate font-mono">{slide.link}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1 shrink-0 ml-2">
                            <button onClick={() => startEditBanner(slide)} className="p-1.5 text-gray-400 hover:text-green-600 rounded-lg"><Edit3 className="w-4 h-4" /></button>
                ...
                <button onClick={addNewBanner} className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-green-200 text-green-600 text-sm font-medium rounded-xl hover:bg-green-50 mt-2 transition-colors">
                  <Plus className="w-4 h-4" /> Add New Slide
                </button>
              </SectionCard>
            )}
            ...
            {activeSection === 'delivery' && (
              <SectionCard key="delivery" title="Delivery Settings" icon={<Truck className="w-5 h-5" />}>
                <Field label="Delivery Fee" hint="Base charge for delivery orders.">
                  <Input type="number" value={draft.deliveryFee} onChange={(v) => updateDraft({ deliveryFee: Number(v) })} prefix={draft.currencySymbol} placeholder="40" />
                </Field>
                <Field label="Free Delivery Threshold" hint="Minimum order value for free delivery.">
                  <Input type="number" value={draft.freeDeliveryThreshold} onChange={(v) => updateDraft({ freeDeliveryThreshold: Number(v) })} prefix={draft.currencySymbol} placeholder="100" />
                </Field>
                <div className="bg-blue-50 rounded-xl p-4 border border-blue-100 space-y-1">
                  <p className="text-xs text-blue-700 leading-relaxed">
                    💡 <strong>Payout Note:</strong> Delivery personnel currently earn 100% of the delivery fee ({draft.currencySymbol}{draft.deliveryFee}) per completed order.
                  </p>
                </div>
              </SectionCard>
            )}
            ...
            {activeSection === 'social' && (
              <SectionCard key="social" title="Social Media Links" icon={<Share2 className="w-5 h-5" />}>
                <Field label="Facebook"><Input value={draft.socialLinks.facebook} onChange={(v) => updateDraft({ socialLinks: { ...draft.socialLinks, facebook: v } })} prefix="🔗" /></Field>
                <Field label="Instagram"><Input value={draft.socialLinks.instagram} onChange={(v) => updateDraft({ socialLinks: { ...draft.socialLinks, instagram: v } })} prefix="🔗" /></Field>
                <Field label="Twitter / X"><Input value={draft.socialLinks.twitter} onChange={(v) => updateDraft({ socialLinks: { ...draft.socialLinks, twitter: v } })} prefix="🔗" /></Field>
              </SectionCard>
            )}
            ...
            {activeSection === 'store' && (
              <SectionCard key="store" title="Store Rules" icon={<ShoppingCart className="w-5 h-5" />}>
                <Toggle enabled={draft.allowGuestCheckout} onToggle={() => updateDraft({ allowGuestCheckout: !draft.allowGuestCheckout })}
                  label="Guest Checkout" description="Allow users to checkout without an account." />
                <div className="pt-4">
                  <Field label="Max Items Per Cart" hint="Limits total quantity of items per order.">
                    <Input type="number" value={draft.maxCartItems} onChange={(v) => updateDraft({ maxCartItems: Number(v) })} placeholder="50" />
                  </Field>
                </div>
              </SectionCard>
            ...
            {activeSection === 'danger' && (
              <SectionCard key="danger" title="Danger Zone" icon={<ShieldAlert className="w-5 h-5" />}>
                <Toggle enabled={draft.maintenanceMode} onToggle={() => updateDraft({ maintenanceMode: !draft.maintenanceMode })}
                  label="Maintenance Mode" description="Disable public access to the storefront." />
                ...
                {draft.maintenanceMode && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                    <p className="text-xs text-amber-700 font-medium">
                      Store is currently locked. Remember to save changes to apply this status live.
                    </p>
                  </motion.div>
                )}

                <div className="pt-6 mt-4 border-t border-gray-100">
                  <h4 className="text-sm font-semibold text-gray-800">Factory Reset</h4>
                  <p className="text-xs text-gray-400 mb-4">Restore all configuration to original system defaults.</p>
                  <button onClick={handleReset}
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${resetConfirm ? 'bg-red-600 text-white shadow-md' : 'bg-red-50 text-red-600 border border-red-100 hover:bg-red-100'}`}>
                    {resetConfirm ? <><Check className="w-4 h-4" /> Click to Confirm Reset</> : <><RotateCcw className="w-4 h-4" /> Reset to Defaults</>}
                  </button>
                </div>
              </SectionCard>
            )}
