...
//   // ✅ Prepend virtual "All" category
//   const allCategory: Category = { _id: 'all', name: 'All' };
...
const CategorySlider = ({ selectedCategory, onSelectCategory }: CategorySliderProps) => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [fetching, setFetching] = useState(true);
  ...
  return (
    ...
    >
      ...
      <div
        className='flex gap-6 overflow-x-auto px-10 pt-4 pb-4 scrollbar-hide scroll-smooth'
        ref={scrollRef}
        onMouseEnter={stopAutoScroll}
        onMouseLeave={startAutoScroll}
      >
        {/* ✅ Map allCategories (includes "All"), not just categories */}
        {allCategories.map((cat, index) => (
          <motion.div
            key={cat._id}
            whileHover={{ scale: 1.05 }}
            onClick={() => onSelectCategory(cat.name)} // ✅ fire the callback
            className={`min-w-39 md:min-w-43 flex flex-col items-center justify-center rounded-2xl
              ${colors[index % colors.length]} shadow-md hover:shadow-xl transition-all cursor-pointer
              ${selectedCategory === cat.name ? 'ring-2 ring-green-500 ring-offset-2 scale-105' : ''}`} // ✅ active ring
          >
            <div className='flex flex-col items-center justify-center p-3 md:p-5'>
              {cat.image ? (
                <div className='relative w-12 h-12 mb-3'>
